document.addEventListener("DOMContentLoaded", () => {
  ...
  brandSelect.addEventListener("change", () => {
    ...
    results.innerHTML =
      `<p class="placeholder">Select a brand and model to view details.</p>`;

    if (!brandSelect.value) return;

    const selectedBrand = brandSelect.value.toLowerCase();

    const models = skiData.filter(
  ...
  modelSelect.addEventListener("change", () => {
    const ski = skiData.find(
      s => s.slug === modelSelect.value
    );
    if (!ski) return;

    results.innerHTML = `
      <div class="card">
        <h2>${ski.brand.toUpperCase()} ${ski.model}</h2>
